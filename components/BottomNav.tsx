...
import { Search, Clock, Zap, Users, PlayCircle } from 'lucide-react';
  ...
  return (
    <div className="fixed bottom-8 left-0 right-0 z-[100] flex justify-center px-4 animate-slide-up stagger-4 pointer-events-none">
      <div className="pointer-events-auto bg-white/40 backdrop-blur-3xl border border-white/20 shadow-[0_20px_50px_rgba(0,0,0,0.1)] py-3 px-3 rounded-[40px] flex items-center gap-3">
        {navItems.map((item) => {
          // Normalize active check to handle sub-views like course-player
          const isActive = activeView === item.id || (item.id === 'courses' && activeView === 'course-player');
          ...
          return (
            ...
            >
              <div className={`
                w-14 h-14 rounded-full flex items-center justify-center transition-all duration-500 ease-[cubic-bezier(0.23,1,0.32,1)]
                ${isActive 
              ...
              {/* Tooltip Label */}
              <span className="absolute -top-10 scale-0 group-hover:scale-100 transition-transform bg-black text-white text-[10px] font-bold uppercase tracking-widest px-3 py-1.5 rounded-full pointer-events-none">
                {item.label}
              </span>
